---
title: Settings reference
description: A reference for the Leaf launcher's Settings menu, grouped by section.
---

A reference for the Settings menu. Open Settings from the launcher; the sections
below mirror its layout.

:::note[Reviewer note]
This page is the most likely to drift as the UI evolves. Confirm section names,
row labels, and available options against the shipped build before release — and
update when the Performance and Bluetooth screens are finalized.
:::

## Appearance

Theme and layout, under **Settings → Appearance**.

- **Theme** — a curated colour-scheme picker: **Leaf** (default), Aurora, Ember,
  Orchid, Slate, Rosé. Selecting one sets all colour roles at once.
- **Colours** — fine-tune individual colour roles (accent, background, text,
  selection, hints, button label, button background) with a colour picker. Editing
  any colour switches the theme to "Custom."
- **Layout** — list/pill **style** (e.g. Rounded, Soft, Square, Leaf), **font
  family**, and **font size**.
- **Status Bar** — toggle the **clock**, **battery**, and **Wi-Fi** indicators, and
  choose the clock style.

## Display & Sound

- **Brightness** — screen backlight level.
- **Volume** — system volume (also adjustable with the hardware volume keys, which
  show an on-screen overlay).

## Wi-Fi

Scan for and join networks, see connection status and signal strength, and turn the
radio on or off. The on/off state persists across reboots.

## Bluetooth

Pair and connect Bluetooth devices such as headsets and controllers.

## Behavior

- **Startup tab** — which tab the launcher opens on.
- **Auto-sleep** — idle timeout before the device sleeps (can be turned off).
- **Button hints** — show or hide the footer hint bar. With hints off, content
  expands to fill the screen.
- **Game Performance** — how hard the device works during gameplay: **Auto**,
  **Balanced**, **Performance**, or **Battery Saver**. See
  [Features → Performance](/leaf-docs/guide/features/).

## About

- **Version** — the installed Leaf version.
- **System Update** — check for, download, and install updates over the air. See
  [Updating (OTA)](/leaf-docs/guide/updating/).
